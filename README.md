    ...
    - Return a `section` element with class `'h-screen grid place-items-center'`.
        ...
        - Inside the `section` element, create a `Form` element with the following attributes:
            ...
            - Use the `FormInput` component twice:
                ...
                - First, for an email input with type `'email'`, label `'email'`, name `'identifier'`, and
...
- Create an `action` function:
    ...
    - The `action` function takes a `store` as a parameter and returns an asynchronous function that takes a `request`
      parameter.
    - Inside the async function:
        ...
        - Get the `user` from the Redux store using `store.getState().userState.user`.
        - Get the `cartItems`, `orderTotal`, and `numItemsInCart` from the Redux store
          using `store.getState().cartState`.
...
- Conditional Rendering:
    ...
    - If there are no orders, return a component, such as `<SectionTitle />`, with a message like 'Please make an
      order'.
...
- Create the `ComplexPaginationContainer` component:
    ...
    - Inside the component:
        ...
        - Create an `addPageButton` function that returns a button element with the appropriate classes and onClick
          handler.
        - Create a `renderPageButtons` function that generates an array of page buttons, including ellipses and the
          current page button.
...
?? === This operator is known as the nullish coalescing operator in JavaScript. It is a logical operator that returns
its right-hand side operand when its left-hand side operand is null or undefined, and otherwise returns its left-hand
side operand.
